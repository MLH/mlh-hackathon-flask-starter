...
class GitHub:
    ...
    def get(self, route_url, params={}):
        ...
        access_token = {'Authorization': 'token ' + self.access_token}

        return requests.get(url, headers=access_token, params=params).json()

    def post(self, route_url, params={}):
        url = api_url + route_url
        params = {'Authorization': 'token ' + self.access_token}

        return requests.post(url, headers=params).json()

    def delete(self, route_url, params={}):
        url = api_url + route_url
        params = {'Authorization': 'token ' + self.access_token}

        return requests.delete(url, headers=params).json()
